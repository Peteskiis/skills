...
short-lived `CCP_SESSION_TOKEN` through the VM's private credential document.
The in-VM ccp wrapper loads and expiry-checks that document for each operation,
so a newly applied token is used without restarting the VM or shell. Re-run
`ccp auth sync --vm <vm_id>` when the current token expires; renewable in-VM
...
- `CCP_API_KEY` and externally supplied `CCP_SESSION_TOKEN` values are used as-is
  ...
  rejects an expired token. A 401 means remint/revoke-check the API key or
  re-sync the VM session token.
